...
const SalaryManagement = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editItem) {
        await api.put(`/api/salaries/${editItem.salaryId}`, form);
      } else {
  ...
  const monthName = (m) => new Date(2026, m - 1)
          ...
          <div style={styles.modal}>
            <div style={styles.modalBox}>
              <h2 style={styles.modalTitle}>
                {editItem ? '✏️ Edit Salary Record' : '➕ Add Salary Record'}
              </h2>
                  ...
                  <div style={styles.formGroup}>
                    ...
                    <select style={styles.input} value={form.role}
                      onChange={e => setForm({
                        ...form,
                        role: e.target.value,
                        baseSalary: defaultSalaries[e.target.value] || form.baseSalary
                      })}>
